...
def _price_summary(prices: pd.DataFrame) -> dict[str, int]:
    if prices.empty:
        return {"usable_60m": 0, "fallback_only": 0, "missing": 0}
    ...
    return {"usable_60m": usable_60m, "fallback_only": fallback_only, "missing": missing}
...
def _add_available_game_times(games: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    game_times_path = (
...
def main() -> None:
    ...
    games = _add_available_game_times(games, args)
