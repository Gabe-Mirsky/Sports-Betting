"""Research-only sweep of YES suppression and NO-only policies."""
...
from strategy.side_suppression import run_side_suppression_research, save_side_suppression_outputs  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sweep side-suppression policies for calibrated signals.")
    ...
    parser.add_argument("--prefix", default="side_suppression")
...
def main() -> None:
    ...
    save_side_suppression_outputs(descriptive, validated, folds, summary, output_dir, prefix=args.prefix)
    print(f"Side suppression status: {summary.get('status', 'n/a')}")
    ...
    print(f"Saved side suppression research to: {output_dir}")
